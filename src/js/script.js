    ...
    options: {
        ...
        onClick: function (e) {
            ...
            if (element.length > 0) {
                ...
                let datasetLabel = this.config.data.datasets[element[0]._datasetIndex].label;
...
// document.getElementById('ctx').onclick = function(e){
...
//         var datasetLabel = myChart.config.data.datasets[0].label;
//         let data = myChart.config.data.datasets[0].data[0];
//         removeData(myChart, 'all');
//         addData(myChart, 'A', data, 'click');
//     }
// }
...
function addData(chart, label, data, id = null) {
    console.log('add with id ', id, 'data ', data);
...
function removeData(chart, id) {
    ...
    chart.data.datasets.forEach((dataset) => {
        if (dataset.data && Array.isArray(dataset.data)) {
            dataset.data = dataset.data.filter(d => {
                return d.id === id
            })
        }
    });
